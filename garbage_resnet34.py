...
#    print(net)
    ...
    for epoch in range(epochs):  
      ...
      #      print('interations:',i)
            ...
            if i % 50 == 49: # 每50个batch打印一下训练状态
                ...
                test_accuracy_count.append((100 * correct / total).item())
                train_accuracy_count.append((100 * train_correct / train_total).item())
    ...
    print("end train！time：%d seconds" % (end - start))
    ...
    plt.title('Resnet34 Test Accuracy')
    plt.legend()
    plt.show()
    ...
    plt.title('Resnet34 Train Accuracy')
    plt.legend()
    plt.show()
    ...
    plt.title('Resnet34 Diff Accuracy')
    plt.legend()
    plt.show()
...
def plot_confusion_matrix(cm_raw, class_names):

    # 按行归一化用于上色
    cm_norm = cm_raw.astype('float') / cm_raw.sum(axis=1)[:, np.newaxis]

    plt.figure(figsize=(8, 6))
    ax = sns.heatmap(cm_norm, annot=cm_raw, fmt='d', cmap='RdYlGn',
                     xticklabels=class_names, yticklabels=class_names,
                     vmin=0, vmax=1)

    # 横向显示标签和坐标轴标题
    ax.set_yticklabels(ax.get_yticklabels(), rotation=0)
    ax.set_ylabel('Actual', rotation=0, labelpad=10)
    ax.set_xlabel('Predicted')

    plt.title('Confusion Matrix\n(Colors = Accuracy, Numbers = Counts)')
    plt.tight_layout()
    plt.show()
'''
def test():
    ...
    net = t.load(model, weights_only=False)
    net.eval()
